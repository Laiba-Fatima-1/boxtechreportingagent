...
/**
 * Currency that never overflows its container.
 *
 * A KPI card cannot fit "$1,989,432" at display size, and truncating to
 * "$1,989,..." destroys the only thing the card exists to show. Above a
 * million we switch to compact notation instead: "$1.99M" is shorter,
 * instantly readable, and still exact enough for a headline figure. The
 * precise value stays available in the underlying report.
 */
export const smartMoney = (n, currency = "USD") => {
  const v = n ?? 0;

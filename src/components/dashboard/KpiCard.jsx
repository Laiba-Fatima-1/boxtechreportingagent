  ...
  return (
    <Card
      ...
      className={`card-hover anim-rise group relative flex flex-col overflow-hidden ${DELAY[Math.min(index, 5)]} ${
        feature ? "kpi-feature text-white" : ""
      }`}
    >
      {/* header: icon + label */}
      <div className="flex items-start gap-2.5 px-4 pt-4">
        ...
        <span
          className={`mt-1.5 min-w-0 flex-1 truncate text-[11px] font-medium uppercase tracking-[0.07em] ${
            feature ? "text-white/60" : "text-ink-faint"
          }`}
        >
          {kpi.label}
        </span>
